...
AppSettings loadAppSettings() {
    ...
    out.log_path = s.value("logs/path", "/tmp/trusttunnel-qt.log").toString();
    ...
    out.routing_cache_path = s.value("routing/cache_path",
            QStandardPaths::writableLocation(QStandardPaths::AppConfigLocation) + "/ru-subnet.lst")
                                     .toString();
    ...
    if (out.log_path.isEmpty()) {
        out.log_path = "/tmp/trusttunnel-qt.log";
    }
    return out;
}
